...
<link href="/demo/Public/admin/css/bootstrap-datetimepicker.min.css" rel="stylesheet" media="screen">
...
<script type="text/javascript" src="/demo/Public/admin/js/bootstrap-datetimepicker.js" charset="UTF-8"></script>
<script type="text/javascript" src="/demo/Public/admin/js/locales/bootstrap-datetimepicker.zh-CN.js" charset="UTF-8"></script>
...
<form action="<?php echo U('Content/edit_media');?>" method="POST">
		<div class="wenjian b">
      ...
      <div class="col-md-8">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control" name='title' value="<?php echo ($media["title"]); ?>" placeholder="请输入文章标题" >
              </div>
               </div>
               <div class="col-md-4">
             ...
             <div class="form-group">
                <label for="exampleInputEmail1">所属栏目</label>
                     ...
                     <?php if(!empty($category_sonlist)): if(is_array($category_sonlist)): $i = 0; $__LIST__ = $category_sonlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo["catid"] == $media[catid]): ?><option value="<?php echo ($vo["catid"]); ?>"  selected="selected"><?php echo ($vo["catname"]); ?></option>
                                    <?php else: ?>
...
<?php if(!empty($media["thumb"])): ?><img src="<?php echo ($media["thumb"]); ?>" class="thumbnail litpic">
                                        <?php else: ?>
            ...
            <p><a href="#" class="btn btn-success" data-toggle="modal" data-target="#gridSystemModal">上传封面图片</a></p>
                      <?php if($media["thumb"] != 'null'): ?><input type="hidden" value="<?php echo ($media["thumb"]); ?>" name='thumb' id='thumb'>
                                        <?php else: ?>
                                        

            <input type="hidden" value="/demo/Public/admin/img/tub.png" name='thumb' id='thumb'><?php endif; ?>
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control" name='keywords' id="exampleInputEmail1" value='<?php echo ($media["keywords"]); ?>' placeholder="请输入文章关键字">
      </div>
        <div class="form-group">
        <label for="exampleInputEmail1">文章描述</label>
        <textarea class="form-control" rows="3" name='description' ><?php echo ($media["description"]); ?></textarea>
      </div>
    </div>
		</div>
                      ...
                      <label for="exampleInputEmail1">资源上传区域</label>
                            ...
                            <div id="import-drop-target" class="bs-dropzone">
                                ...
                                <ul id="files_media">
                                 ...
                                 <?php if(is_array($imglist)): $i = 0; $__LIST__ = $imglist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['add'] == 1): ?><li><a target="_blank" href="<?php echo ($vo["link"]); ?>" class="updone"><div class="mediaimg" style="  background-image: url('<?php echo ($vo["link"]); ?>');"></div><a href="javascript:void(0)" class='updelete'>删除</a><span><?php echo ($vo["name"]); ?></span><input type="hidden" name="filetype[]" value="<?php echo ($vo["type"]); ?>"><input type="hidden" name="fileadd[]" value="1"><textarea name="filename[]" class="miaoshu" value="<?php echo ($vo["name"]); ?>"><?php echo ($vo["name"]); ?></textarea><input type="hidden" name="filelink[]" value="<?php echo ($vo["link"]); ?>">
...
<div class="clear-fix"></div>
   <label for="exampleInputEmail1">网络资源区</label>
     ...
     <?php if(!empty($imglist)): if(is_array($imglist)): $i = 0; $__LIST__ = $imglist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['add'] == 2): ?><div class="row weblist">
                                  <div class="col-xs-6">
                                    <input type="text" class="form-control" name='filelink[]' value="<?php echo ($vo["link"]); ?>" placeholder="请输入文件网址">
                                  </div>
                                  <div class="col-xs-2">
                                    <select class="form-control"  name='filetype[]' > 
                                    <option value=''>请选择</option>
                                      <?php if($vo['type'] == 'video'): ?><option value="video" selected="selected">视频</option> 
                                   <option value="music">音乐</option>  
                                   <?php else: ?>
                                         <option value="video">视频</option> 
                                   <option value="music"  selected="selected">音乐</option><?php endif; ?>           
                                   </select>
                                  </div>
                                  <div class="col-xs-4">
                                    <input type="text" class="form-control"name='filename[]'  value="<?php echo ($vo["name"]); ?>"  placeholder="请输入文件描述">
                                      <input type="hidden" class="form-control fileadd" name='fileadd[]'  value="2">

                                  </div>
                                  <a  href="javasrcipt:void('0')" class="deleteweb"><img src="/demo/Public/admin/img/updelete.png"></a>
...
<div class="row">
                                ...
                                <p><a href="javascript:void(0)" class="addone pull-right">再追加一条</a></p><?php endif; ?>
			  ...
			  <textarea id="editor" placeholder="Balabala" autofocus name='content' class="weditor" style="height:400px;max-height:2500px;"><?php echo ($media["content"]); ?></textarea>
             ...
             <div class="wenjian b">
                  ...
                  <div class="col-sm-9">
                    <div class="checkbox">
                      ...
                      <label><input type="checkbox" name='posids[]' value="2" checked="checked"> 推荐到本栏目</label><?php endif; ?>
                         <?php if($media["posids"] == '3'): ?><label>
                        <input type="checkbox" name='posids[]' value="1" checked="checked"> 推荐到首页
                      </label>
                      <label><input type="checkbox" name='posids[]' value="2" checked="checked"> 推荐到本栏目</label><?php endif; ?>
                         <?php if($media["posids"] == '0'): ?><label>
                        <input type="checkbox" name='posids[]' value="1" > 推荐到首页
                      </label>
                      <label><input type="checkbox" name='posids[]' value="2"> 推荐到本栏目</label><?php endif; ?>
                    </div>
                ...
                <div class="row showqi" style="display:block">
                    <div class="col-sm-4">
                             <div class="form-group">
                                      ...
                                      <input class="form-control form_datetime"   name='inputtime' placeholder="请填写发布时间" value="<?php echo ($media["updatetime"]); ?>">
                            </div>
                        </div>
                          <div class="col-sm-4">
                             <div class="form-group">
                                      <label for="message-text" class="control-label">排序:</label>
                                      <input class="form-control"    name='listorder' placeholder="请填写排序顺序" value="<?php echo ($media["listorder"]); ?>">
                            </div>
                        </div>
                        ...
                        <div class="col-sm-4">
                             <div class="form-group">
                                      ...
                                      <input class="form-control"   name='url' placeholder="请填写要跳转到的站外链接" value="<?php echo ($media["url"]); ?>">
                            </div>
                  </div>
              ...
              <input type="hidden" value="<?php echo ($media["id"]); ?>" name='id'></input>
		      <div class="wenjian b">
