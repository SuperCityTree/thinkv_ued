...
/**
 * idtrans($id,$tables,$zid)
 * id转换函数
 * @parma $id 要转换的id $tables 字段所在表  $zid 转换的字段
 * @author linzi <linzi2519@sina.com>
 * @version $Id: common.php  $
 */
function idtrans($id,$tables,$zid){
    $res=M($tables)->where('id='.$id)->getField($zid);
    return $res;        
}
function get_duration($file,$rate=128){
    //数据量=（采样频率×采样位数×声道数×时间）/8 
  if(!$file)return ;
  /*return(filesize($file)/1024/1024);
  return ceil((filesize($file)*8)/44.1/1000/16/2);*/
